...
describe("embedded-markdown", () => {
	...
	test("getEmbeddedMarkdownSections with heredoc sections", (t) => {
		...
		const expected = [
			{ "markdown": "# Hello\n\nThis is a paragraph.", "lineOffset": 2, "firstLineColumnOffset": 0, "columnOffset": 2 },
			{ "markdown": "Says hello.", "lineOffset": 7, "firstLineColumnOffset": 0, "columnOffset": 2 }
		];
	...
	test("getEmbeddedMarkdownSections with CRLF sections", (t) => {
		...
		const text = "defmodule MyApp do\r\n  @moduledoc \"\"\"\r\n  # Hello\r\n  \"\"\"\r\nend\r\n";
		const expected = [
			{ "markdown": "# Hello", "lineOffset": 2, "firstLineColumnOffset": 0, "columnOffset": 2 }
		];
		const actual = getEmbeddedMarkdownSections(text, "elixir", { "elixir": [ heredocPattern ] });
		t.assert.deepEqual(actual, expected);
	});
	...
	test("getEmbeddedMarkdownSections with whitespace-only line", (t) => {
		...
		const expected = [
			{ "markdown": "# H\n\nbody", "lineOffset": 1, "firstLineColumnOffset": 0, "columnOffset": 2 }
		];
	...
	test("getEmbeddedMarkdownSections with single-line section at document start", (t) => {
		...
		const expected = [
			{ "markdown": "Hello", "lineOffset": 0, "firstLineColumnOffset": 6, "columnOffset": 0 }
		];
		const actual = getEmbeddedMarkdownSections(text, "elixir", { "elixir": [ pattern ] });
		t.assert.deepEqual(actual, expected);
	});
	...
	test("getEmbeddedMarkdownSections with multiple patterns in document order", (t) => {
		...
		const expected = [
			{ "markdown": "One", "lineOffset": 1, "firstLineColumnOffset": 0, "columnOffset": 0 },
			{ "markdown": "Two", "lineOffset": 3, "firstLineColumnOffset": 10, "columnOffset": 0 },
			{ "markdown": "Three", "lineOffset": 5, "firstLineColumnOffset": 0, "columnOffset": 0 }
		];
	...
	test("adjustResults with offsets", (t) => {
		t.plan(1);
		const section = { "markdown": "# Hello\nworld", "lineOffset": 10, "firstLineColumnOffset": 0, "columnOffset": 2 };
		/** @type {any[]} */
		const results = [
	...
	test("adjustResults with first-line column offset", (t) => {
		t.plan(1);
		const section = { "markdown": "Hello", "lineOffset": 5, "firstLineColumnOffset": 8, "columnOffset": 0 };
		const results = [
